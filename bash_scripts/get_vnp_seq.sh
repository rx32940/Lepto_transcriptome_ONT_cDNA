...
cat $WORK/$sample/position_after_3_prime.bed | grep $'\tCDS\t' | grep $'\t+\t' | grep -e "partial(3')" > $WORK/$sample/CDS_after_3_prime.forward.bed
cat $WORK/$sample/position_after_3_prime.bed | grep $'\tCDS\t' | grep $'\t-\t' | grep -e "partial(3')"  > $WORK/$sample/CDS_after_3_prime.reverse.bed

cat $WORK/$sample/position_before_3_prime.bed | grep $'\tCDS\t' | grep $'\t+\t' | grep -e "partial(3')" > $WORK/$sample/CDS_before_3_prime.forward.bed
cat $WORK/$sample/position_before_3_prime.bed | grep $'\tCDS\t' | grep $'\t-\t' | grep -e "partial(3')"  > $WORK/$sample/CDS_before_3_prime.reverse.bed
...
bedtools getfasta -fi $ref/$REF.fna -bed $WORK/$sample/CDS_after_3_prime.forward.bed -fo $WORK/$sample/$sample.CDS.after.forward.fasta
bedtools getfasta -fi $ref/$REF.fna -bed $WORK/$sample/CDS_after_3_prime.reverse.bed -fo $WORK/$sample/$sample.CDS.after.reverse.fasta

bedtools getfasta -fi $ref/$REF.fna -bed $WORK/$sample/CDS_before_3_prime.forward.bed -fo $WORK/$sample/$sample.CDS.before.forward.fasta
bedtools getfasta -fi $ref/$REF.fna -bed $WORK/$sample/CDS_before_3_prime.reverse.bed -fo $WORK/$sample/$sample.CDS.before.reverse.fasta
