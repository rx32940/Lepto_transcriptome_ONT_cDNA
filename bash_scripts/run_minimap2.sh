...
#SBATCH --job-name=map_full_length
#SBATCH --ntasks=1
...
OUT="/scratch/rx32940/minION/polyA_cDNA/map_full"
...
#     samclip --max 10 --ref $ref/$REF.fna < $OUT/sam/$sample.sam > $OUT/sam/$sample.noClip.sam\n
#     samtools view -bS $OUT/sam/$sample.noClip.sam > $OUT/bam/$sample.bam \n
#     samtools view -@ 3 -bS $OUT/sam/$sample.noClip.sam | samtools sort - -@ 3 -o $OUT/bam/$sample.sorted.bam\n 
...
SAM='/scratch/rx32940/minION/polyA_cDNA/map_full/sam'
OUT="/scratch/rx32940/minION/polyA_cDNA/map_full"
for file in $SAM/*PolyATail.sam;
do
ml BEDTools/2.30.0-GCC-8.3.0
ml SAMtools/1.10-GCC-8.3.0
ml BEDTools/2.30.0-GCC-8.3.0
sample=$(basename $file ".sam")
samtools view -@ 3 -bS $file | samtools sort - -@ 3 | bedtools bamtobed -i - -cigar -tag NM > $OUT/bed/$sample.withClip.bed
